...
5. **增强错误反馈与多端一致性优化**
   - **显式错误告警**: 现在当路线规划受阻（如“模式暂不支持”或“未找到路线”）时，错误信息会直穿卡片，在原有的距离/时间位置显式输出，不再只是简单的隐藏，方便快速定位问题。
   - **实时状态同步**: 增加了出行方式的 `watch` 监听。一旦用户手动切换了交通工具，之前计算的过期距离/耗时或残存的报错信息会立即被“一键归零”，确保页面上的小图标与文字内容永远保持同步。
   - **视觉统一**: 在 `shared/common.css` 中新增了通用的 `.card-route-error` 样式规范，确保三端在报错展示上的视觉风格高度一致且醒目。
