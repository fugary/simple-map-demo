...
### 3.4 多端 API 兼容性处理
**【重要规则】**本项目需同时兼容在纯 Web 浏览器环境与 Tauri 桌面端环境中运行。在调用特定 API（如 `window.__TAURI__` 或是原生 Web 接口）时，必须执行预先的环境和存在性校验（例如判断 `window.__TAURI__ != null`）。这能有效防止因缺失环境依赖而引发的全局 JS 崩溃。
